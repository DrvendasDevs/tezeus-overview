...
export function WhatsAppProvidersConfig({ workspaceId, workspaceName }: WhatsAppProvidersConfigProps) {
  ...
  const [zapiStatusWebhook, setZapiStatusWebhook] = useState('');
  ...
  useEffect(() => {
    ...
    if (zapiProvider) {
      ...
      setZapiStatusWebhook(zapiProvider.n8n_status_webhook_url || '');
  ...
  const handleSaveZapi = async () => {
    ...
    try {
      ...
      const providerData = {
        ...
        n8n_status_webhook_url: zapiStatusWebhook?.trim() || undefined,
        ...
        <TabsContent value="zapi" className="space-y-4">
          <Card className="dark:bg-[#0b0b0b] dark:border-gray-700">
            ...
            <CardContent className="space-y-4 dark:text-gray-100">
              ...
              <div className="space-y-2">
                <Label htmlFor="zapi-webhook">Webhook N8N - Mensagens (Opcional)</Label>
                <Input
                  id="zapi-webhook"
                  placeholder="https://n8n.example.com/webhook/..."
                  value={zapiWebhook}
                ...
                />
                <p className="text-xs text-muted-foreground">
                  URL para receber mensagens e eventos gerais
                </p>
              </div>

              <div className="space-y-2">
                <Label htmlFor="zapi-status-webhook">Webhook N8N - Status (Opcional)</Label>
                <Input
                  id="zapi-status-webhook"
                  placeholder="https://n8n.example.com/webhook/status..."
                  value={zapiStatusWebhook}
                  onChange={(e) => setZapiStatusWebhook(e.target.value)}
                  className="dark:bg-[#1a1a1a] dark:border-gray-700 dark:text-gray-100 dark:placeholder:text-gray-400"
                />
                <p className="text-xs text-muted-foreground">
                  URL para receber callbacks de status de mensagens (enviado, entregue, lido)
                </p>
              </div>
